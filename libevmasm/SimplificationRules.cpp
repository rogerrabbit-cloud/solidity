...
AssemblyItem Pattern::toAssemblyItem(yul::DebugData::Ptr const& _debugData) const
{
	if (m_type == Operation)
		return AssemblyItem(m_instruction, _debugData);
	else
		return AssemblyItem(m_type, data(), _debugData);
}
...
ExpressionTemplate::ExpressionTemplate(Pattern const& _pattern, yul::DebugData::Ptr const& _debugData)
{
