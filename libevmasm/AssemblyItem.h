...
public:
	...
	AssemblyItem(u256 _push, yul::DebugData::Ptr _debugData = {}):
		AssemblyItem(Push, std::move(_push), std::move(_debugData)) { }
	AssemblyItem(Instruction _i, yul::DebugData::Ptr _debugData = {}):
		m_type(Operation),
		m_instruction(_i),
		m_debugData(std::move(_debugData))
	{}
	AssemblyItem(AssemblyItemType _type, u256 _data = 0, yul::DebugData::Ptr _debugData = {}):
		m_type(_type),
		m_debugData(std::move(_debugData))
	{
		if (m_type == Operation)
	...
	void setLocation(langutil::SourceLocation const& _location) {
		if (!m_debugData)
			m_debugData = yul::DebugData::create(_location);
		else
			m_debugData->nativeLocation = _location;
	}
	...
	void setDebugData(yul::DebugData::Ptr const& _debugData) { m_debugData = _debugData; }
	yul::DebugData::Ptr debugData() const { return m_debugData; }
