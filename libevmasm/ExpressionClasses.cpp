...
ExpressionClasses::Id ExpressionClasses::newClass(yul::DebugData::Ptr const& _debugData)
{
	Expression exp;
	exp.id = static_cast<Id>(m_representatives.size());
	exp.item = storeItem(AssemblyItem(UndefinedItem, (u256(1) << 255) + exp.id, _debugData));
	m_representatives.push_back(exp);
	m_expressions.insert(exp);
	return exp.id;
}
