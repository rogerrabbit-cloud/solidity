...
public:
	...
	Id stackElement(int _stackHeight, yul::DebugData::Ptr _debugData);
	/// @returns the stackElement relative to the current stack height.
	Id relativeStackElement(int _stackOffset, yul::DebugData::Ptr _debugData = {});
...
private:
	...
	void swapStackElements(int _stackHeightA, int _stackHeightB, yul::DebugData::Ptr _debugData);

	/// Increments the sequence number, deletes all storage information that might be overwritten
	/// and stores the new value at the given slot.
	/// @returns the store operation, which might be invalid if storage was not modified
	StoreOperation storeInStorage(Id _slot, Id _value,yul::DebugData::Ptr _debugData);
	/// Retrieves the current value at the given slot in storage or creates a new special sload class.
	Id loadFromStorage(Id _slot, yul::DebugData::Ptr _debugData);
	/// Increments the sequence number, deletes all memory information that might be overwritten
	/// and stores the new value at the given slot.
	/// @returns the store operation, which might be invalid if memory was not modified
	StoreOperation storeInMemory(Id _slot, Id _value, yul::DebugData::Ptr _debugData);
	/// Retrieves the current value at the given slot in memory or creates a new special mload class.
	Id loadFromMemory(Id _slot, yul::DebugData::Ptr _debugData);
	/// Finds or creates a new expression that applies the Keccak-256 hash function to the contents in memory.
	Id applyKeccak256(Id _start, Id _length, yul::DebugData::Ptr _debugData);
