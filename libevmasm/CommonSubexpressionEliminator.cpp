...
std::vector<AssemblyItem> CommonSubexpressionEliminator::getOptimizedItems()
{
	...
	for (int height = minHeight; height <= m_initialState.stackHeight(); ++height)
		initialStackContents[height] = m_initialState.stackElement(height, yul::DebugData::create());
	for (int height = minHeight; height <= m_state.stackHeight(); ++height)
		targetStackContents[height] = m_state.stackElement(height, yul::DebugData::create());
...
void CommonSubexpressionEliminator::optimizeBreakingItem()
{
	...
	if (*m_breakingItem == AssemblyItem(Instruction::JUMPI))
	{
		...
		Id condition = m_state.stackElement(m_state.stackHeight() - 1, yul::DebugData::create(itemLocation));
		if (classes.knownNonZero(condition))
		{
			feedItem(AssemblyItem(Instruction::SWAP1, yul::DebugData::create(itemLocation)), true);
			feedItem(AssemblyItem(Instruction::POP, yul::DebugData::create(itemLocation)), true);

			AssemblyItem item(Instruction::JUMP, yul::DebugData::create(itemLocation));
			item.setJumpType(jumpType);
			m_breakingItem = classes.storeItem(item);
		}
		else if (classes.knownZero(condition))
		{
			AssemblyItem it(Instruction::POP, yul::DebugData::create(itemLocation));
			feedItem(it, true);
			feedItem(it, true);
			m_breakingItem = nullptr;
		}
	...
	{
		Id size = m_state.stackElement(m_state.stackHeight() - 1, yul::DebugData::create(itemLocation));
		if (classes.knownZero(size))
		{
			feedItem(AssemblyItem(Instruction::POP, yul::DebugData::create(itemLocation)), true);
			feedItem(AssemblyItem(Instruction::POP, yul::DebugData::create(itemLocation)), true);
			AssemblyItem item(Instruction::STOP, yul::DebugData::create(itemLocation));
			m_breakingItem = classes.storeItem(item);
		}
	}
}
...
void CSECodeGenerator::generateClassElement(Id _c, bool _allowSequenced)
{
	...
	yul::DebugData::Ptr const& itemDebugData = expr.item->debugData();
...
void CSECodeGenerator::appendDup(int _fromPosition, yul::DebugData::Ptr const& _debugData)
{
	assertThrow(_fromPosition != c_invalidPosition, OptimizerException, "");
	int instructionNum = 1 + m_stackHeight - _fromPosition;
	assertThrow(instructionNum <= 16, StackTooDeepException, util::stackTooDeepString);
	assertThrow(1 <= instructionNum, OptimizerException, "Invalid stack access.");
	appendItem(AssemblyItem(dupInstruction(static_cast<unsigned>(instructionNum)), _debugData));
	m_stack[m_stackHeight] = m_stack[_fromPosition];
	m_classPositions[m_stack[m_stackHeight]].insert(m_stackHeight);
}

void CSECodeGenerator::appendOrRemoveSwap(int _fromPosition, yul::DebugData::Ptr const& _debugData)
{
	assertThrow(_fromPosition != c_invalidPosition, OptimizerException, "");
	...
	assertThrow(instructionNum <= 16, StackTooDeepException, util::stackTooDeepString);
	assertThrow(1 <= instructionNum, OptimizerException, "Invalid stack access.");
	appendItem(AssemblyItem(swapInstruction(static_cast<unsigned>(instructionNum)), _debugData));
